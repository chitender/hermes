...
- **Reconcile storm:** `Status().Update()` re-enqueues the CR. Fixed by comparing old vs new status before updating (commit `6a202e8`).
- **Watcher restart loop:** Watcher was being restarted on every reconcile. Fixed by comparing watcher config (paths, interval) and reusing if unchanged (commit `69b1251`).
- **Helm CRD ownership:** CRD in `crds/` dir is not labeled by Helm. Moved to `templates/` with `helm.sh/resource-policy: keep` annotation, then pre-stamped ownership labels on standalone manifest (commits `e1d2a8a`, `3d9516f`).
- **Projected volume issues:** Splitting K8s API token and Vault token into separate projected volumes is required — Vault needs audience `vault`, but controller-runtime needs the default audience. Also must include `ca.crt` and `namespace` in the kube-api-access volume (commits `95a6fe9`, `d8b5924`).
- **json.Number type:** Vault returns numbers as `json.Number`, not `int`/`float64`. Must handle type assertion in `toInt()` helper (commit `64fa539`).
